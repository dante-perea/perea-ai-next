...
export function McpDocs({ serverUrl, secret }: McpDocsProps) {
  const addCommand = `claude mcp add --transport http perea-knowledge-base ${serverUrl} --header "Authorization: Bearer ${secret}"`;

  const mcpJson = JSON.stringify(
    {
      mcpServers: {
        "perea-knowledge-base": {
          type: "http",
          url: serverUrl,
    ...
    <div className="space-y-8 max-w-3xl">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--color-ink-muted)]">
          Connect Claude Code to your knowledge base to list files, fetch content, and build knowledge graphs.
        </p>
      </div>

      <Section title="Quick add — Claude Code CLI">
        <p className="text-sm text-[var(--color-ink-muted)]">
          Run this once in any terminal. Claude Code will register the server globally.
        </p>
      ...
      <Section title="Alternative — .mcp.json (per-project)">
        <p className="text-sm text-[var(--color-ink-muted)]">
          Drop a <span className="font-mono text-xs">.mcp.json</span> at the root of any project. Claude Code picks it up automatically.
        </p>
        <CodeBlock code={mcpJson} language="json" />
        <p className="text-xs text-[var(--color-ink-faint)]">
          Add <span className="font-mono">.mcp.json</span> to <span className="font-mono">.gitignore</span> — it contains your secret.
        </p>
      </Section>
      ...
      <Section title="Example workflows">
        <div className="space-y-4">
          <div>
            ...
            <CodeBlock
              code={`List all files in my knowledge base, download each one, and save them to the artifacts/ folder.`}
              language="prompt"
            />
          </div>
          <div>
            ...
            <CodeBlock
              code={`Download all files tagged "research" from my knowledge base to artifacts/.`}
              language="prompt"
            />
          </div>
          <div>
            ...
            <CodeBlock
              code={`Download all knowledge base files to artifacts/, then run graphify on the folder to build a knowledge graph.`}
              language="prompt"
            />
          </div>
        </div>
        ...
        <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-white)] p-4 space-y-2 text-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs text-[var(--color-ink)]">{serverUrl}</span>
              <CopyButton text={serverUrl} />
            </div>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[var(--color-ink-faint)]">Auth</span>
            <span className="font-mono text-xs text-[var(--color-ink-muted)]">Bearer token (MCP_SECRET)</span>
          </div>
          <div className="flex items-center justify-between">
