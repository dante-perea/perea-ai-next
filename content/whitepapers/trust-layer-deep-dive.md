...
As of mid-2026, that stack has crystallized around three primitives: SD-JWT+KB as defined in IETF RFC 9901,[^1] AP2's Mandate model as published by Google's `google-agentic-commerce/AP2`,[^2] and the PEP/PDP authorization split popularized by Microsoft, Cordum, and the OPA community.[^3][^4]

AP2 was launched in September 2025 alongside payments and technology partners as an open protocol for initiating and transacting agent-led payments.[^24] The broader analyst frame casts it as the "trust and governance layer" of agentic commerce that complements an execution-layer protocol like x402 and a merchant-integration protocol like ACP.[^25]
...
AP2's Section 4 ("Trust Anchors: Verifiable Digital Credentials & Mandates")[^13] defines three Verifiable Digital Credentials (VDCs) — Cart Mandate, Intent Mandate, Payment Mandate — secured as SD-JWT+KB credentials. The newer specification documentation collapses two of these into the umbrella term "Checkout Mandate."[^14] Fintech analyst commentary frames the Mandate model as the first rail-agnostic protocol that gives merchants and banks "dispute-grade proof of approval" — a signed packet showing the user approved a specific cart, valid across cards, real-time bank transfers, and stablecoins.[^26] Either way, the protocol distinguishes:
...
**Payment Mandate.** Provides the Credential Provider, Network, and Merchant Payment Processor cryptographic proof that the Shopping Agent is authorized to pay for a particular Checkout.[^14] The Payment Mandate is bound to a particular Checkout via the cryptographic hash of the Checkout JWT.[^14] Independent commentary describes the Payment Mandate as a "minimal credential derived from the cart or intent" that gives payment networks visibility into agent involvement and the type of approval without exposing more data than needed,[^31] and frames the three-mandate structure as "boring on purpose" because it relies on explicit, signed statements rather than inference about user intent.[^31][^32]
...
The chasm Sonnet & Prose called out — and AP2's verification rules implicitly encode — is between cryptographic proof and business authorization. A signature is verifiable. Authority is not. The cryptographic chain proves *what was signed*; it does not prove *that the signer was allowed to sign it*. A January 2026 arXiv red-teaming evaluation makes this concrete by demonstrating "Branded Whisper" and "Vault Whisper" indirect-prompt-injection attacks against an AP2-based shopping agent built on Gemini 2.5 + the Google ADK framework — mandates provided cryptographic guarantees over what was authorized, but the agent's interpretation of conversational context could still be steered to invoke them in unintended ways.[^23] Independent analyst commentary reaches the same conclusion from the security operations side: AP2 is not a "trust by default" switch, and runtime verification, prompt isolation, and explicit delegated-authority controls are mandatory for production-worthy use.[^27]
...
AWS Prescriptive Guidance for multi-tenant SaaS authorization frames OPA as the canonical PDP option (Cedar via Amazon Verified Permissions is the other), and explicitly recommends a centralized PDP with PEPs at every API endpoint as the baseline architecture.[^28] AWS additionally documents both a centralized-PDP and a distributed-PDP-with-policy-sync deployment topology, with the distributed pattern preferred for low-latency or partition-tolerant systems where every PDP must still produce identical decisions for identical inputs.[^34]

Independent definitions of the PDP role converge on the same shape: the PDP returns ALLOW/DENY plus obligations and metadata, and the PEP enforces the decision and any obligations it carries.[^29][^33] CapiscIO's RFC-005 PDP Integration Profile formalizes the wire contract between PEPs and external PDPs: the PEP enforces fail-closed if the PDP cannot validate constraint-narrowing, which the RFC names as "the authority escalation guardrail" of the delegation model.[^35]
...
[^23]: Wang, Mengjie et al. "Red-Teaming Google's Agent Payments Protocol via Prompt Injection." arXiv:2601.22569. January 30, 2026. https://arxiv.org/html/2601.22569v1
[^24]: The Paypers. "Google rolls out Agent Payments Protocol." September 17, 2025. https://thepaypers.com/payments/news/google-launches-agent-payments-protocol-ap2
[^25]: Wang, Tom. "Google AP2 Completes the Agentic Payment Stack." April 3, 2026. https://tomcn.uk/news/2026-04-03-google-ap2-agentic-payment-protocol-stack
[^26]: Taylor, Simon. "Google Agent to Payment (AP2) Explained." Fintech Brainfood. September 21, 2025. https://www.fintechbrainfood.com/p/ap2-explained
[^27]: Blue Headline. "AP2 Security: How AI Agent Payments Are Reshaping Financial Security In 2026." April 14, 2026. https://blueheadline.com/cybersecurity/ap2-security-2026-ai-agent-payments-2/
[^28]: Amazon Web Services. "Implementing a PDP by using OPA — AWS Prescriptive Guidance." Accessed May 2026. https://docs.aws.amazon.com/prescriptive-guidance/latest/saas-multitenant-api-access-authorization/opa.html
[^29]: DevSecOps School. "What is PDP? Meaning, Architecture, Examples, Use Cases (2026 Guide)." February 20, 2026. https://devsecopsschool.com/blog/pdp/
[^30]: AgentPayTrend. "AP2 x402: 5 AI Agent Payments Shifts." April 20, 2026. https://agentpaytrend.com/ap2-x402-ai-agent-payments-shifts/
[^31]: Adam Holter. "Google's Agent Payments Protocol (AP2): A Practical Primer." September 17, 2025. https://adam.holter.com/googles-agent-payments-protocol-ap2-a-practical-primer/
[^32]: ContentBase. "AP2 Mandates Explained for AI Payments." April 18, 2026. https://contentbase.com/blog/ap2-mandates-guide/
[^33]: Big ACL. "Policy Decision Point (PDP) — Runtime Authorization Engine." Accessed May 2026. https://big-acl.com/policy-decision-point
[^34]: AWS Prescriptive Guidance. "Design models for OPA — Centralized vs Distributed PDPs." Accessed May 2026. https://docs.aws.amazon.com/prescriptive-guidance/latest/saas-multitenant-api-access-authorization/using-opa.html
[^35]: CapiscIO. "RFC-005: PDP Integration Profile (PIP)." February 25, 2026. https://docs.capisc.io/rfcs/005-policy-definition-distribution-enforcement/
